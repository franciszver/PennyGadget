...
def create_beta_users(db: Session):
    ...
    existing_beta = db.query(User).filter(User.cognito_sub.like("beta-%")).all()

    if existing_beta:
        print(
            f"[INFO] Found {len(existing_beta)} existing beta users, skipping creation"
        )
