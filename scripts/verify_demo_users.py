...
def verify_goals(
    db: Session, user_id, expected_completed: int = 0, expected_active: int = 0
) -> bool:
    """Verify user has expected goals"""
    completed = (
        db.query(Goal)
        .filter(Goal.student_id == user_id, Goal.status == "completed")
        .count()
    )

    active = (
        db.query(Goal)
        .filter(Goal.student_id == user_id, Goal.status == "active")
        .count()
    )
...
def verify_sessions(db: Session, user_id, min_count: int) -> bool:
    ...
    count = db.query(SessionModel).filter(SessionModel.student_id == user_id).count()
    return count >= min_count


def verify_user_age(
    db: Session, user: User, expected_days: int, tolerance: int = 1
) -> bool:
...
def test_progress_endpoint(base_url: str, user_id: str) -> Dict:
    ...
    try:
        ...
        response = requests.get(f"{base_url}/api/v1/progress/{user_id}", timeout=5)
        if response.status_code == 200:
            data = response.json()
            return {
                "success": True,
                "has_suggestions": "suggestions" in data.get("data", {}),
            }
        else:
    ...
    with get_db_session() as db:
        ...
        for email, expectations in demo_accounts.items():
            ...
            if not verify_user_age(db, user, expectations["expected_age_days"]):
                print(
                    f"  [ERROR] User age incorrect (expected ~{expectations['expected_age_days']} days ago)"
                )
                all_passed = False
            else:
            ...
            if not verify_goals(
                ...
                print(
                    f"  [ERROR] Goals incorrect (expected {expectations['expected_completed_goals']} completed, {expectations['expected_active_goals']} active)"
                )
                all_passed = False
            else:
            ...
            if max_sessions:
                ...
                session_count = (
                    ...
                    .filter(SessionModel.student_id == user.id)
                    .count()
                )
                if session_count != max_sessions:
                    print(
                        f"  [ERROR] Session count incorrect (expected exactly {max_sessions}, got {session_count})"
                    )
                    all_passed = False
                else:
                ...
                if not verify_sessions(db, user.id, min_sessions):
                    print(
                        f"  [ERROR] Session count too low (expected at least {min_sessions})"
                    )
                    all_passed = False
                else:
                    session_count = (
