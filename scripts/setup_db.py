    ...
    try:
        ...
        try:
            ...
            if "already exists" in error_msg:
                ...
                print(
                    "    [INFO] Full execution failed, trying statement by statement..."
                )
                conn.rollback()
        ...
        if "already exists" in error_msg or "duplicate" in error_msg:
            print(
                f"    [INFO] Migration partially applied (some objects already exist)"
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--skip-create-db", action="store_true", help="Skip database creation"
    )
...
def create_initial_migration(migrations_dir):
    ...
    schema_doc = (
        Path(__file__).parent.parent / "_docs" / "active" / "DATABASE_SCHEMA.md"
    )
