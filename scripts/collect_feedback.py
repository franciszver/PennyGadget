...
def generate_feedback_report(db: Session):
    ...
    if engagement_7d["engagement_rate"] < 50:
        report["recommendations"].append(
            "Low engagement - consider improving onboarding or feature discoverability"
        )

    if engagement_7d["practice_completed"] == 0:
        report["recommendations"].append(
            "No practice completion - check practice assignment flow"
        )

    if engagement_7d["qa_queries"] == 0:
        report["recommendations"].append(
            "No Q&A usage - check Q&A interface accessibility"
        )
